const SIZES_IN_MM:Record<string, number[]> = {
    'A0': [841, 1189],
    'A1': [594, 841],
    'A2': [420, 594],
    'A3': [297, 420],
...
const mmToM = function(dimension: number[]):number[] {
    if (dimension) {
        return [
...
const mmToCM = function(dimension: number[]):number[] {
    if (dimension) {
        return [
...
const mmToInches = function(dimension: number[] | undefined):number[] {
    if (dimension) {
        return [
...
const mmToPixel = function(dimension: number[] | undefined, dpi: number):number[] {
    dpi || (dpi = 72);
...
export = {
    register: function(name: string, width: number, height: number) {
        if (width > height) {
            height = [width, width = height][0];
        }
        SIZES_IN_MM[name.toUpperCase()] = [width, height];
    },
    getSize: function(name: string, options: { unit?: any; dpi?: any; }) {
        options || (options = {});
        let dimension = SIZES_IN_MM[name.toUpperCase()];
        if (options.unit) {
